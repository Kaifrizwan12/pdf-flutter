...
void openWebBlob({
  ...
  iframe.onLoad.first.then((_) {
    try {
      ...
      js_util.callMethod<void>(contentWindow, 'focus', const []);
      js_util.callMethod<void>(contentWindow, 'print', const []);
    } catch (_) {
      fallback();
    }
  }).catchError((_) {
...
bool _printViaPreparedWindow(Object printTarget, String url) {
  ...
  void fallback() {
    ...
    usedFallback = true;
    try {
      js_util.setProperty(targetWindow, 'location', url);
    } catch (_) {
      html.window.open(url, '_blank');
    }
  }
  ...
  if (doc is! html.HtmlDocument) {
    try {
      js_util.callMethod<void>(targetWindow, 'close', const []);
    } catch (_) {}
    return false;
  }
